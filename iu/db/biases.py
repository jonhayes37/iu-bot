...
ULTIMATE_BIAS_COLUMNS = {"name", "birth_name", "birthday", "colour", "group_name", "hometown",
                         "image_filename", "position", "reason"}
ARTIST_BIAS_COLUMNS = {"name", "album", "b_track", "bias", "colour", "debut_date", "image_filename", "label",
                       "members", "reason", "title_track"}
...
def create_ultimate_bias_db(user_id: int, name: str, birth_name: str, birthday: str,
    ...
    """Inserts a new ultimate bias record for a user. Returns False if they already have one."""
    try:
...
def _check_columns(updates: dict, allowed: set[str]):
    """The column names go straight into the SQL text, so only known ones are accepted."""
    unknown = set(updates) - allowed
    if unknown:
        raise ValueError(f"Unknown bias columns: {sorted(unknown)}")
...
def update_ultimate_bias_db(user_id: int, **updates) -> bool:
    ...
    _check_columns(updates, ULTIMATE_BIAS_COLUMNS)
    if not updates:
        return False
...
def create_artist_bias_db(
    ...
    """Inserts a new artist bias (bias group) record for a user. Returns False if they already have one."""
    try:
        with db_connection(Database.BIASES) as conn:
            conn.execute("""
...
def update_artist_bias_db(user_id: int, **updates) -> bool:
    ...
    _check_columns(updates, ARTIST_BIAS_COLUMNS)
    if not updates:
        return False
