...
from db.merch import credit_hearts
...
def create_tournament(name: str, description: str, entrants: List[str], days_per_round: int) -> str:
    """
    Creates a tournament, entrants, and the linked match tree in one transaction, and returns the
    new tournament's ID. Nothing is created if any part fails.
    """
...
def get_unpolled_matches(tournament_id: str, round_num: int) -> list[dict]:
    """
...
def set_match_poll_data(match_id: int, message_id: int, end_time: datetime) -> None:
    """
...
def get_expired_unresolved_matches() -> list[dict]:
...
def advance_winner(match_id: int, tournament_id: str, current_round: int, current_pos: int, winner_id: int) -> None:
...
def get_tournament_days(tournament_id: str) -> int:
    ...
    Defaults to 1 for a tournament that doesn't exist.
    """
    with db_connection(Database.TOURNAMENTS) as conn:
        cursor = conn.cursor()
        cursor.execute(
...
class RoundReward:
    """A heart that was just paid for voting in every matchup of a round."""
    tournament_id: str
    tournament_name: str
    round_num: int

def record_vote(message_id: int, user_id: int, answer_id: int) -> tuple[str, int] | None:
    """
    Saves a user's vote using the exact entrant_id. Returns (tournament_id, round_num), or None if
    the poll isn't a tournament match (this event fires for every poll in the server).
    """
    with db_connection(Database.TOURNAMENTS, row_factory=True) as conn:
        cursor = conn.cursor()
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?)
        """, (match['match_id'], user_id, choice_entrant_id))

        return match['tournament_id'], match['round_num']

def claim_round_reward(tournament_id: str, round_num: int, user_id: int) -> RoundReward | None:
    """
    Pays the user a heart if they have now voted in every matchup of the round and haven't been
    paid for it yet. Returns the reward, or None if nothing was paid.

    The payment (merch database) and the ledger entry that stops it being paid twice (tournaments
    database) happen in one transaction, so a user is never paid without being marked as paid or
    marked as paid without being paid.
    """
    with db_connection(Database.TOURNAMENTS, row_factory=True, attach=(Database.MERCH,)) as conn:
        # Take the write lock up front, so two votes at once can't both be paid
        conn.execute("BEGIN IMMEDIATE")

        already_paid = conn.execute("""
            SELECT 1 FROM tournament_rewards_ledger WHERE tournament_id = ? AND round_num = ? AND user_id = ?
        """, (tournament_id, round_num, user_id)).fetchone()
        if already_paid:
            return None

        # Check total matches in the round vs. the user's votes in it
        total_matches = conn.execute("""
            SELECT COUNT(*) FROM tournament_matches WHERE tournament_id = ? AND round_num = ?
        """, (tournament_id, round_num)).fetchone()[0]
        ...
        """, (tournament_id, round_num, user_id)).fetchone()[0]
        ...
        conn.execute("""
            INSERT INTO tournament_rewards_ledger (tournament_id, round_num, user_id) VALUES (?, ?, ?)
        """, (tournament_id, round_num, user_id))
        credit_hearts(conn, "IU bot", user_id, 1,
                      f"Voted in every matchup for round {round_num} of **{tournament_name}**", schema="merch.")

        return RoundReward(tournament_id, tournament_name, round_num)

def process_user_vote(message_id: int, user_id: int, answer_id: int) -> RoundReward | None:
    """Saves a vote, then pays the round reward if that vote completed the round. None if nothing was paid."""
    recorded = record_vote(message_id, user_id, answer_id)
    if recorded is None:
        return None
    return claim_round_reward(*recorded, user_id)
