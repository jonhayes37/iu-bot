...
**Why Seohyun (서현)?**
She was my first bias, and the start of it all! A talented musician, angelic vocalist, stunning visual and hilarious troublemaker <:snsd:795873457086791741>""")

@pytest.mark.asyncio()
@pytest.mark.parametrize(["user", "member", "expected_message", "expected_embed", "expected_file"],
[
    # User doesn't have command unlocked
    (not_unlocked_user, "", f"Sorry <@!{not_unlocked_user.id}>, you haven't unlocked an ultimate bias yet!", None, None),
    # Non-existent member
    (main_user, "Soshified", f"Sorry <@!{main_user.id}>, Soshified hasn't unlocked an ultimate bias yet!", None, None),
