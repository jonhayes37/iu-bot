...
- **Schema changes:** `CREATE TABLE IF NOT EXISTS` won't alter existing tables. Add new columns by adding an entry to `COLUMN_MIGRATIONS` in `db/initialize.py` (see the `tournaments.description` example) in addition to editing the `.sql`. New indexes can just be `CREATE INDEX IF NOT EXISTS` in the schema.
